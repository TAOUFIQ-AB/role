...
class BrowserManager:
    def __init__(self):
        ...
        if Config.WEBSHARE_ENABLED and Config.WEBSHARE_API_KEY:
            try:
    ...
    def launch(self, cookies: List[dict] = None) -> None:
        ...
        ua = Config.BROWSER_USER_AGENT

        self.log.info(
            "Launching Chromium (headless=%s, ua=%s)...",
            Config.HEADLESS,
            "override" if ua else "native",
        )
        self._pw = sync_playwright().start()
        _chromium_exe = os.environ.get("PLAYWRIGHT_CHROMIUM_EXECUTABLE_PATH", "").strip()
        _browser_channel = Config.BROWSER_CHANNEL if not _chromium_exe else ""
        _launch_selector = {}
        if _chromium_exe:
            _launch_selector["executable_path"] = _chromium_exe
        elif _browser_channel:
            _launch_selector["channel"] = _browser_channel
        ...
        # Keep only stable language preference. Chromium supplies its own
        # version-correct Client Hints and fetch headers.
