...
class ReelTask:
    ...
    def mark_retry(self, reason: str, kind: FailureKind = FailureKind.TRANSIENT) -> None:
        # Record the failure kind before evaluating is_retryable; otherwise a
        # first failure sees failure_kind=None and is incorrectly marked FAILED.
        self.attempt += 1
    ...
    def flush_retries(self) -> int:
        ...
        while True:
            try:
                ...
                # mark_retry() already counted the failed attempt. Moving the
                # task between queues must not consume another retry budget.
